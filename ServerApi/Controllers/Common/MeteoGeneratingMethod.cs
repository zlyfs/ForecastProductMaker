...
{
    public class MeteoGeneratingMethod
    {

        public static string MeteoFMV(string modelText, MissionInfo missionInfo)
        {
            //替换海浪预报数据{ fmv}
            var forecastList = ReshapeFMV(missionInfo);
            int i = 0;
            try
            {
                foreach (var temp in forecastList)
                {
                    modelText = ForecastReplace.Replace(modelText, "{fmv}", temp);
                    i++;
                }
            }
            catch (Exception e)
